...
select id, pname from full_passengers where id between 591 and 883;
...
select pname, survived from full_passengers where pname like '%(alfrida%';
...
select ticket, count(id) from full_passengers group by ticket order by count(id) desc limit 1;
...
select embarked 登船點, count(id) 登船人數, round(count(id)/(select count(id) from full_passengers)*100, 2) 登船點占百分比 from full_passengers where embarked != "" group by embarked;